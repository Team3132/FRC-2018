...
get_usb_partition() {
    ...
    MOUNT_POINT=$(mount |grep /dev/sdb1|sed -e 's/^.* on \([^ ]*\).*/\1/') 
    echo
    ...
    if [ "$FIRST_DIR" != "/media" ] && [ "$FIRST_DIR" != "/mnt" ]&& [ "$FIRST_DIR" != "/tmp" ]; then
    	echo "Unexpected mount point, giving up for safety"
    	exit 3
    fi 
    echo
...
check_partition_is_ext4() {
	PARTITION=$1
	FORMAT=$(mount |grep /dev/sdb1|sed -e 's/^.* type \([^ ]*\).*/\1/')
...
main() {
    ...
    # Ensure that copy completes by waiting for umount to finish. It will wait for all copies.
    echo "umounting..."
